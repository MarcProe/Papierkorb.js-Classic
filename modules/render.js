let render = {
    rendercallback: function (err, req, res, template, data, conf, title) {
        ...
        let promiseLoadUsers = new Promise(function (resolve, reject) {
            if (req.session.userlist && req.session.userlistobj) {
                resolve();
            } else {
                ...
                req.app.locals.db
                    ...
                    .toArray(function (err, userlistres) {
                        if (err) {
                            ...
                            req.session.userlistobj = {};
                            req.session.userlist.forEach((o) => {
                                req.session.userlistobj[o._id] = {};
                                req.session.userlistobj[o._id].icon = o.icon;
                            });
                            resolve();
                        }
                    });
